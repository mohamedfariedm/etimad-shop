...
function Faq({homeData,FaqeData}:any) {
  const { t,i18n } = useTranslation("homepage");
  const [activeButtons, setActiveButtons] = useState<number[]>([]);
  console.log("FaqeData",FaqeData)
  ...
  return (
    <div className="main-container flex w-full lg:w-[1152px] flex-col gap-[32px] items-start flex-nowrap relative mx-auto my-0">
      <div className="w-full lg:w-[1152px] h-[374px] relative mx-auto my-0">
        <div className="w-full lg:w-[1152px] h-[374px] bg-[url(/assets/images/faq/frame.png)]  bg-cover bg-no-repeat rounded-[32px] absolute top-0 end-0" />
        <div className="flex w-full lg:w-[466px] h-[150px] flex-col gap-[24px] flex-nowrap absolute top-[112px] lg:end-[576px] z-[1] max-xs:mx-5">
          <span className="h-[36px] self-stretch shrink-0 basis-auto text-[24px] font-semibold leading-[36px] text-[#fff] relative text-start whitespace-nowrap z-[2]">
            {homeData?.data?.sections[4]?.title}
          </span>
          <span className="flex w-full lg:w-[466px] h-[90px] items-start self-stretch shrink-0 text-[16px] font-medium leading-[30.4px] text-[#f8fbfc] relative text-start z-[3] max-xs:w-[80%]">
      ...
      <div className="flex gap-[24px] items-start self-stretch shrink-0 flex-wrap relative z-[1] transition-all duration-1000 ease-in-out">
        {FaqeData?.data?.map((item:any, index:any) => (
          <button
            key={index}
              ...
              <div className="transition-all duration-1000 ease-in-out w-full">
                ...
                <div className="w-full flex justify-between items-center relative">
                  <span className="flex w-[211px] h-[30px] items-start shrink-0 basis-auto text-[16px] font-medium leading-[30px] text-[#5d9d9f] relative text-start whitespace-nowrap z-[2]">
                    {item?.title_question?.[currentLanguage]}
                  </span>
                  <div
                ...
                <span
                  ...
                  dangerouslySetInnerHTML={{
                    __html: activeButtons.includes(index) ? item?.answer?.[currentLanguage] || "" : "",
                  }}>
                  {/* {activeButtons.includes(index) ? item?.answer?.[currentLanguage] : ""} */}
                </span>
              </div>
            </>
          </button>
