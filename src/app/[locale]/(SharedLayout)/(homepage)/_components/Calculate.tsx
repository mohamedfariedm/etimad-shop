...
function Calculate({homeData}:any) {
  const { t } = useTranslation("homepage");
  const [loanAmount, setLoanAmount] = useState<number>(1000);
  const [duration, setDuration] = useState<number>(6);

  // Ensure interest rate is 0%
  const interestRate = 0.0;

  // Calculate Monthly Installment (without interest)
  const monthlyInstallment = (loanAmount / duration).toFixed(2);

  // Total amount to be paid (same as loan amount since no interest)
  const totalAmount = (loanAmount).toFixed(2);

  // Total profit for the lender (should always be 0 since there's no interest)
  ...
  return (
    <div className="main-container flex w-full flex-col xl:flex-row xl:w-[1152px] gap-[24px] items-center flex-nowrap relative mx-auto  my-0">
      ...
      <div
          className="self-stretch grow shrink-0 bg-cover bg-no-repeat rounded-[24px] relative z-[53]"
          style={{
            backgroundImage: `url(${homeData?.data?.sections[2]?.Posts[0].attachment[0]?.original || "/assets/images/calculate/hero.svg"})`
          }}
        />

        <div className="flex w-full xl:w-[516px] xl:h-[120px] flex-col gap-[24px] absolute bottom-[64px] left-1/2 translate-x-[-50%] z-[54] max-xs:rtl:left-40 max-xs:ltr:left-52">
          ...
          <span className="flex w-full xl:w-[516px] xl:h-[60px] text-[16px] font-medium leading-[30.4px] text-[#f8fbfc] relative z-[56] max-xs:w-[86%]">
            {homeData?.data?.sections[2]?.Posts[0]?.title}
          </span>
        </div>
      </div>
